...
class ProviderManager:
    ...
    def __init__(self, provider_config=None):
        ...
        if provider_config:
            ...
            if provider_config.did:
                self.video_providers.append(provider_config.did)
                logger.info("D-ID video provider disponible (Priority 2)")
            if provider_config.higgsfield:
                self.video_providers.append(provider_config.higgsfield)
                logger.info("Higgsfield video provider disponible (Priority 3)")
        
        self.heygen = provider_config.heygen if provider_config else None
        self.did = provider_config.did if provider_config else None
    ...
    def _compose_videos_with_overlaps(
        ...
        video_filter = (
            ...
            f"trim=start=0:end={intro_cut_duration}, setpts=PTS-STARTPTS [v0]; "
            f"[1:v] scale=1080:1920:flags=lanczos, setpts=PTS-STARTPTS [v1]; "
            f"[2:v] scale=1080:1920:flags=lanczos, setpts=PTS-STARTPTS [v2]; "
    ...
    def _process_with_did(
        ...
        """
        Procesa video usando D-ID (video completo con TTS + lip-sync integrado).
        
        D-ID requiere audio pre-generado, por lo que:
        1. Genera audio con ElevenLabs para frame 3
        2. Usa D-ID para generar video con imagen y audio
        ...
        Raises:
            Exception: Si D-ID o ElevenLabs no están disponibles o fallan
        """
        if not self.did:
            raise Exception("D-ID no está disponible")
        if not self.elevenlabs:
            raise Exception("ElevenLabs no está disponible (requerido para generar audio)")
        
        logger.info(f"[{video_id}] Procesando con D-ID + ElevenLabs...")
        ...
        main_video_path = temp_dir / "main_video_did.mov"
        main_video = self.did.generate_video(
            script_frame3,
            avatar_id="",
            output_path=main_video_path,
            audio_path=audio_frame3,
        ...
        output_path = temp_dir / "video_final_did.mp4"
        return self._compose_videos_with_overlaps(
            intro_with_audio_path,
            main_video,
            outro_video,
    ...
    def _process_with_elevenlabs(
        ...
        audio_duration = float(json.loads(audio_duration_result.stdout)["format"]["duration"])
        
        video_duration_result = subprocess.run([
            "ffprobe", "-v", "error", "-show_entries", "format=duration",
            "-of", "json", str(base_video)
        ], capture_output=True, text=True, check=True)
        video_duration = float(json.loads(video_duration_result.stdout)["format"]["duration"])
    ...
    def process_video_with_fallback(
        ...
        2. D-ID (Video completo - TTS + lip-sync integrado)
        3. Higgsfield (Video completo - TTS + lip-sync integrado)
        4. Wav2Lip (TTS con ElevenLabs + lip-sync con Wav2Lip)
        5. ElevenLabs (TTS + base video sin lip-sync - fallback final)
        ...
        Args:
            ...
            base_video: Path to base video (frame 3 - Santa) - usado en Wav2Lip, ElevenLabs (no usado en D-ID)
        ...
        if self.did and self.elevenlabs:
            try:
                logger.info(f"[{video_id}] Intentando con D-ID + ElevenLabs (Priority 2)...")
                result = self._process_with_did(
                    intro_video, outro_video, script_frame2, script_frame3,
                    temp_dir, video_id
                )
                shutil.copy2(result, output_path)
                logger.info(f"[{video_id}] ✅ Video generado exitosamente con D-ID")
                return output_path
            except Exception as e:
                last_error = e
                strategies_tried.append("D-ID")
                logger.warning(f"[{video_id}] D-ID falló: {str(e)}")
        
        if self.higgsfield:
            try:
                logger.info(f"[{video_id}] Intentando con Higgsfield (Priority 3)...")
        ...
        if self.wav2lip and self.elevenlabs:
            try:
                logger.info(f"[{video_id}] Intentando con Wav2Lip + ElevenLabs (Priority 4)...")
        ...
        if self.elevenlabs:
            try:
                logger.info(f"[{video_id}] Intentando con ElevenLabs (Priority 5 - fallback final)...")
